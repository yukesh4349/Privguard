...
// ========================
const getToken = () => {
    const u = JSON.parse(localStorage.getItem('privguard_user'));
...
const API = {
    simulate: () => fetch('/api/v1/events/simulate', { method: 'POST', headers: getHeaders() }).then(r => r.json()),
    alerts: () => fetch('/api/v1/dashboard/alerts', { headers: getHeaders() }).then(r => r.json()),
    stats: () => fetch('/api/v1/dashboard/stats', { headers: getHeaders() }).then(r => r.json()),
...
async function fetchFlaggedUsers() {
    try {
        const response = await fetch('/api/v1/users/flagged-users', { headers: getHeaders() });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const wsUrl = `${wsProto}//${window.location.host}/api/v1/dashboard/ws`;
    
    const connectWebSocket = () => {
        const ws = new WebSocket(wsUrl);
        ws.onmessage = (event) => {
            fetchLiveAlerts(); // Re-fetch cleanly on trigger to ensure full sync
        };
        ws.onclose = () => {
